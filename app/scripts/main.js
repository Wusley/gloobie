  ...
  Fish.prototype.calc = function() {
    ...
    if (this.following != null) {
      ...
      if (this.angleDirectionDifference() > (Math.PI * 0.9) && // On colliding angle?
        ...
        if (keyDown) affinityLine(this.following, this, "blue");
      ...
      if (keyDown) affinityLine(this.following, this, "green");
    }
    ...
    if (py(this.x - cursor.x, this.y - cursor.y) < FOLLOW_DISTANCE * 0.75) {
      ...
      maxSpeed = 4;
      if (keyDown) affinityLine(cursor, this, "gray");
    }
      ...
      for (var i = 0; i < this.following.entourage.length; i++) {
        ...
        if (siblingFish !== this) {
          if (py(this.x - siblingFish.x, this.y - siblingFish.y) < FOLLOW_DISTANCE * 0.2) {
            if (keyDown) affinityLine(siblingFish, this, "pink");
...
fishBitmap.src = "http://s14.postimg.org/k2xnbkykh/fish.png";

// https://dl.dropboxusercontent.com/u/4534978/2014/fishes/fish.png
...
function draw(f) {
  ...
  context.drawImage(fishBitmap, 0, 0, w, h);
  context.rotate(-r);
  context.translate(-f.x, -f.y);
}
